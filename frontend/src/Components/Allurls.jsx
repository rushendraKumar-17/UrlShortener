...
const Allurls = () => {
  ...
  useEffect(() => {
    if (!logged) return;
    axios
      .get("https://urlshortener-p7ma.onrender.com/api/url", {
        headers: {
  ...
  const handleDelete = (id) => {
    ...
    axios
      .delete(`https://urlshortener-p7ma.onrender.com/api/url/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    ...
    <Box sx={{ padding: "2rem", textAlign: "center" }}>
      ...
      {urls && urls.length > 0 ? (
        <div style={{ display: "flex", flexWrap: "wrap",justifyContent:"space-between" ,gap:"5vh"}}>
          {urls.map((url, index) => (
              <Card sx={{ boxShadow: 3 }} id={url._id} style={{width:"34vw"}}>
                <CardContent>
      ...
      {infoWindow && <InfoWindow handleClose={handleClose} item={selected}/>}
      {deleteWindow && <DeleteWindow handleDelete={handleDelete} item={selected} handleClose={handleClose}/>}
    </Box>):
      (
        <Box
